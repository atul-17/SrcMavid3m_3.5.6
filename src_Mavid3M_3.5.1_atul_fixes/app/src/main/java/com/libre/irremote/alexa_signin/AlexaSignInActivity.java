...
public class AlexaSignInActivity extends AppCompatActivity implements View.OnClickListener, BleWriteInterface {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (getIntent() != null) {
            ...
            LibreLogger.d(this, "suma in getting setup detail ALEXASIGN-IN\n" + speakerIpaddress + "from which activity\n" + fromWhichActivity);


//            mDeviceProvisioningInfo = (DeviceProvisioningInfo) getIntent().getSerializableExtra("deviceProvisionInfo");
        }
        //   MavidApplication.alexaSignINExcecuted=true;
        ...
        if (i == R.id.btn_login_amazon) {
            if (deviceFwVersionList.size() == 5) {
                ...
                if (!deviceFwVersionList.get(4).isEmpty()) {
                    ...
                    uiRelatedClass.buildSnackBarWithoutButton(AlexaSignInActivity.this,
                            getWindow().getDecorView().findViewById(android.R.id.content).getRootView(), "Something went wrong!!");
                }
            } else {
                uiRelatedClass.buildSnackBarWithoutButton(AlexaSignInActivity.this,
                        getWindow().getDecorView().findViewById(android.R.id.content).getRootView(), "Something went wrong!!");
