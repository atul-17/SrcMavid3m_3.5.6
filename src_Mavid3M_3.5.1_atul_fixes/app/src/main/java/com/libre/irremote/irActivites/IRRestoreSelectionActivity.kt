...
class IRRestoreSelectionActivity : AppCompatActivity() {
    ...
    private fun showCustomAlertForDeleteConfirmation(modelRemoteDetails: ModelRemoteDetails) {
        uiRelatedClass.showCustomAlertDialogForDeleteConfirmation(
                this@IRRestoreSelectionActivity,
                object : OnButtonClickCallback {
                            ...
                            deleteUserDevice(getSharedPreferences("Mavid", Context.MODE_PRIVATE).getString("sub", "")!!,
                                ...
                                override fun onSucessFailureCallbacks(isSucess: Boolean, modelRemoteDetails: ModelRemoteDetails?) {
                                    if (isSucess) {
                                        ...
                                        deleteRemoteDetailsOnTheDevice(deviceInfo!!.ipAddress, modelRemoteDetails!!)
    ...
    private fun getRemoteIndexInPrefs(modelRemoteDetails: ModelRemoteDetails): Int {
        ...
        for (i in 0 until modelRemoteSubAndMacDetils.modelRemoteDetailsList.size) {
            if ((modelRemoteDetails.index == modelRemoteSubAndMacDetils.modelRemoteDetailsList[i].index) &&
                    (modelRemoteDetails.selectedAppliance.equals(modelRemoteSubAndMacDetils.modelRemoteDetailsList[i].selectedAppliance)))
                return i
        }
    ...
    /** LDAPI#0 */
    fun deleteRemoteDetailsOnTheDevice(ipAddress: String, modelRemoteDetails: ModelRemoteDetails) {
                ...
                if (modelRemoteSubAndMacDetils.mac == macId) {
                    ...
                    while (iterator.hasNext()) {
                        ...
                        if (storedRemoteDetails.remoteId == modelRemoteDetails?.remoteId
                                && storedRemoteDetails.index == modelRemoteDetails.index
                                && storedRemoteDetails.brandId == modelRemoteDetails?.brandId) {
                            /** Since therer is possiblity that the user can add two devices with same remoteId and brandId,
                             * but it will have different index , while deleting it is necessary to check the index as well.
                             * */
                            Log.d(TAG, "deletedFromSharedPref:".plus(" brandName: ${storedRemoteDetails.selectedBrandName}"
                                    .plus(" remoteId: ${storedRemoteDetails.remoteId}".plus(" index: ${storedRemoteDetails.index}"))))
