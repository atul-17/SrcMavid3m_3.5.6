...
//        BleCommunication.writeInteractor(b);
    ...
    private HashMap<String, String> getDeviceInformation(Scanner bleDeviceInfoScanner, HashMap<String, String> bleDeviceInformationMap) {
        while (bleDeviceInfoScanner.hasNext()) {
            String message = bleDeviceInfoScanner.nextLine();
            String[] dataSplitArr = message.split(":");
            if (dataSplitArr.length < 2) {
                continue;
            }

            bleDeviceInformationMap.put(dataSplitArr[0], dataSplitArr[1]);
        }
        return bleDeviceInformationMap;
    }
    ...
    public void onMessageEvent(BLEEvntBus event) {
        ...
        if (!alreadyExecuted) {
            ...
            if (response2 == 17) {
                ...
                String bleDataRecived = new String(value, StandardCharsets.UTF_8);

                Scanner bleDeviceInfoScanner = new Scanner(bleDataRecived);


                HashMap<String, String> deviceInfoHashmap = getDeviceInformation(bleDeviceInfoScanner, new HashMap<String, String>());


                LibreLogger.d(this, "ble_data_received: " + deviceInfoHashmap);
...
//                BleCommunication.writeInteractorStopSac();
                    ...
                    if (!alreadySignINShown) {
                        ...
                        Intent intent = new Intent(MavidBLEConfigureSpeakerActivity.this, AlexaSignInActivity.class);
                        intent.putExtra("speakerIpaddress", MavidApplication.mBLESACDeviceAddress);
                        intent.putExtra("appComparisonVersion", deviceInfoHashmap.get("AppCompVer"));
                        BleCommunication.writeInteractorStopSac();
