...
class UIRelatedClass {
    ...
    fun showCustomAlertWhileFirmWareIsUpdating(appCompatActivity: AppCompatActivity) {
        if (!appCompatActivity.isFinishing) {
            ...
            val tvAlertTitle: AppCompatTextView = alert.findViewById(R.id.tv_alert_title)

            val tvAlertMessage: AppCompatTextView = alert.findViewById(R.id.tv_alert_message)

            val btnOk: AppCompatButton = alert.findViewById(R.id.btn_ok)

            tvAlertTitle.text = "Updating"

            tvAlertMessage.text = "Smart IR is upgrading to the\n latest firmware"
    ...
    fun showCustomAlertThatAppNeedsToBeUpdated(appCompatActivity: AppCompatActivity) {
        if (!appCompatActivity.isFinishing) {
            ...
            tvAlertTitle.text = "App is not updated"

            tvAlertMessage.text = "Please go to Play Store and update the app\n to the latest version for the feature to work."
...
//                buidCustomSnackBarWithButton(appCompatActivity,"","OK",appCompatActivity)
    ...
    fun getCustomNamesHashMapOfAllRemotes(modelRemoteDetailsList: MutableList<ModelRemoteDetails>): HashMap<String, String> {
        var customNamesHashMap = HashMap<String, String>()

        for (modelRemoteDetails: ModelRemoteDetails in modelRemoteDetailsList) {
            customNamesHashMap[modelRemoteDetails.customName] = modelRemoteDetails.remoteId
        }
        ...
        if (modelRemoteDetailsString != null) {
            ...
            if (modelRemoteDetailsString!!.isNotEmpty()) {
                ...
                if (modelRemoteSubAndMacDetils.mac == macId) {
                    ...
                    if (getCustomNamesHashMapOfAllRemotes(modelRemoteSubAndMacDetils.modelRemoteDetailsList).contains(userEditedName.toUpperCase())) {
                        return false
                    }
...
//                    }
    ...
    fun showCustomAlertDialogForDeleteConfirmation(appCompatActivity: AppCompatActivity,
                                                   onButtonClickCallback: OnButtonClickCallback, deleteMessage: String) {
        ...
        if (!appCompatActivity.isFinishing) {
            ...
            tv_alert_message.text = "This will delete  your already configured $deleteMessage. "
